...
public class ModifyPlayerListener implements OnClickListener {
	private ModifyPlayerActivity activity;

	public ModifyPlayerListener(ModifyPlayerActivity playerActivity) {
	...
	public void onClick(View v) {
		...
		//pseudo a change mais existe deja
		if(! pseudo.equals(activity.getCurrentPlayer().getPseudo())) {
		...
		Player player = new Player(activity.getCurrentPlayer().getId(), pseudo, ImageUtils.getByteArray(bitmap), true);
		PlayerRepositoryCache.update(player, activity);

		//transmission du joueur a l'activite appelante
		Intent intent = new Intent();
		intent.putExtra(Constantes.MODIFIED_PLAYER, player);
